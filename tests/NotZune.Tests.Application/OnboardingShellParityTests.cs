...
public class OnboardingShellParityTests : IDisposable
{
    ...
    {
        _tempDir = Path.Combine(Path.GetTempPath(), $"notzune-onboard-{Guid.NewGuid():N}");
    ...
    public void WhatsNew_Continue_PersistsSeenVersion()
    {
        ...
        Assert.Equal(NotZune.Application.AppInfo.VersionDisplay, vm.VersionDisplay);
        Assert.NotEmpty(vm.Highlights);

        vm.ContinueCommand.Execute(null);

        Assert.Equal(NotZune.Application.AppInfo.Version, _settingsStore.Load().WhatsNewSeenVersion);
    }
