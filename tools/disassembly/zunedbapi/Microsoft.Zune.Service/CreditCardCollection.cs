	...
	internal unsafe int Init(ICreditCardCollection* pCollection)
	{
		...
		if (0 < num2)
		{
			do
			{
				...
				if (num >= 0)
				{
					num = ((delegate* unmanaged[Stdcall, Stdcall]<IntPtr, int, EBillingPaymentType*, ushort**, ushort**, ushort**, ushort**, ushort**, ushort**, ushort**, ushort**, ushort**, ushort**, ECreditCardType*, ushort**, ushort**, ushort**, ushort**, int>)(int)(*(uint*)(*(int*)pCollection + 16)))((nint)pCollection, num3, &eBillingPaymentType, &ptr, &ptr2, &ptr3, &ptr4, &ptr5, &ptr6, &ptr7, &ptr8, &ptr9, &ptr10, &creditCardType, &ptr11, &ptr12, &ptr13, &ptr14);
					if (num >= 0)
					{
