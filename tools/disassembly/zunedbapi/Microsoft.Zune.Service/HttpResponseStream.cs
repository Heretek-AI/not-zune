...
public class HttpResponseStream : Stream
{
	...
	public unsafe HttpResponseStream(IStream* pStream, long length)
	{
		try
		{
			...
			m_pStream = pStream;
			((delegate* unmanaged[Stdcall, Stdcall]<IntPtr, uint>)(int)(*(uint*)(*(int*)pStream + 4)))((nint)pStream);
			_length = length;
			return;
		}
		catch
	...
	public unsafe override int Read(byte[] buffer, int offset, int count)
	{
		...
		if (count + offset > (nint)buffer.LongLength)
		{
		...
		{
			uint num = 0u;
			int num2 = *(int*)m_pStream + 12;
			int num3 = ((delegate* unmanaged[Stdcall, Stdcall]<IntPtr, void*, uint, uint*, int>)(int)(*(uint*)num2))((nint)m_pStream, ptr, (uint)count, &num);
			if (num3 < 0)
			{
