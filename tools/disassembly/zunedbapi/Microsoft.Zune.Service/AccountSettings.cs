...
public class AccountSettings
{
	...
	internal unsafe AccountSettings(INewsletterSettings* pNewsletterSettings, IPrivacySettings* pPrivacySettings)
	{
		...
		if (pNewsletterSettings == null || pPrivacySettings == null || ((delegate* unmanaged[Stdcall, Stdcall]<IntPtr, NewsletterOptions*, int>)(int)(*(uint*)(*(int*)pNewsletterSettings + 16)))((nint)pNewsletterSettings, &newsletterOptions) < 0)
		{
			return;
		}
		m_emailFormat = *(EmailFormat*)(&newsletterOptions);
		m_allowZuneEmails = ((Unsafe.As<NewsletterOptions, int>(ref Unsafe.AddByteOffset(ref newsletterOptions, 4)) != 0) ? true : false);
		m_allowPartnerEmails = ((Unsafe.As<NewsletterOptions, int>(ref Unsafe.AddByteOffset(ref newsletterOptions, 8)) != 0) ? true : false);
		int num = ((delegate* unmanaged[Stdcall, Stdcall]<IntPtr, int>)(int)(*(uint*)(*(int*)pPrivacySettings + 16)))((nint)pPrivacySettings);
		int num2 = 0;
		if (0 >= num)
		{
			return;
		}
		Unsafe.SkipInit(out EPrivacySettingId key);
		Unsafe.SkipInit(out EPrivacySettingValue value);
		while (((delegate* unmanaged[Stdcall, Stdcall]<IntPtr, int, EPrivacySettingId*, EPrivacySettingValue*, int>)(int)(*(uint*)(*(int*)pPrivacySettings + 20)))((nint)pPrivacySettings, num2, &key, &value) >= 0)
		{
