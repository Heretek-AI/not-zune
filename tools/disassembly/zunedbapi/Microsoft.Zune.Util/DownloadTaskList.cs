...
public class DownloadTaskList : AggregateList
{
	...
	private DownloadTaskList()
		: base((IList)DownloadManager.Instance.CompletedDownloads, (IList)DownloadManager.Instance.CancelledDownloads, (IList)DownloadManager.Instance.FailedDownloads, (IList)DownloadManager.Instance.ActiveDownloads)
	{
		try
		{
	...
	{
		int num = ((ListDataSet)m_cancelledDownloadTasks).Count + ((ListDataSet)m_failedDownloadTasks).Count;
		return ((ListDataSet)m_completedDownloadTasks).Count + num;
	}
