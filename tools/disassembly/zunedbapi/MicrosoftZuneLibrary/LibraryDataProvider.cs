...
public class LibraryDataProvider
{
	...
	{
		return (DataProviderQuery)(object)new LibraryDataProviderQuery(queryTypeCookie);
	}
	...
	public unsafe static bool ActOnItems(IList mediaList, BulkItemAction action, EventArgs args)
	{
		if (mediaList != null && mediaList.Count != 0)
		{
			...
			if (num < mediaList.Count)
			{
				do
				{
					if (mediaList[num2] is IDatabaseMedia databaseMedia2)
					{
			...
			if (list.Count > 0)
			{
				...
				case BulkItemAction.UnexcludeFromSync:
					flag = (byte)(((((SyncEventArgs)args).Device.Rules.Unexclude(list.ToArray(), mediaType) >= 0) ? 1u : 0u) & 1u) != 0;
					break;
				case BulkItemAction.ExcludeFromSync:
					flag = (byte)(((((SyncEventArgs)args).Device.Rules.Exclude(list.ToArray(), mediaType) >= 0) ? 1u : 0u) & 1u) != 0;
					break;
				case BulkItemAction.RemoveSyncRules:
					flag = (byte)(((((SyncEventArgs)args).Device.Rules.Remove(list.ToArray(), mediaType, fDeviceFolderIds: false) >= 0) ? 1u : 0u) & 1u) != 0;
					break;
				case BulkItemAction.AddSyncRules:
					flag = (byte)(((((SyncEventArgs)args).Device.Rules.Add(list.ToArray(), mediaType) >= 0) ? 1u : 0u) & 1u) != 0;
				...
				case BulkItemAction.ReverseSync:
				{
					...
					flag = (byte)((action != BulkItemAction.DeleteFromDevice) ? (((((SyncEventArgs)args).Device.ReverseSync(list.ToArray(), mediaType, ref operationStatus) >= 0) ? 1u : 0u) & 1u) : (((((SyncEventArgs)args).Device.DeleteMedia(list.ToArray(), mediaType, ref operationStatus) >= 0) ? 1u : 0u) & 1u)) != 0;
	...
	public static bool GetSortAttributes(string sortString, out string[] sorts, out bool[] ascendings)
	{
		...
		{
			int num = (sorts = sortString.Split(new char[1] { ',' })).Length;
			if (num > 0)
			{
	...
	private static void LogAction(BulkItemAction action, EMediaTypes mediaType, int count)
	{
		if (mediaType == EMediaTypes.eMediaTypeUserCard)
		{
			...
			case BulkItemAction.RemoveSyncRules:
			case BulkItemAction.ExcludeFromSync:
				global::_003CModule_003E.SQMAddWrapper("SocialSyncDeleteProfileCard", count);
				break;
			case BulkItemAction.AddSyncRules:
			case BulkItemAction.UnexcludeFromSync:
				global::_003CModule_003E.SQMAddWrapper("SocialSyncAddProfileCard", count);
				break;
			}
		}
	}
