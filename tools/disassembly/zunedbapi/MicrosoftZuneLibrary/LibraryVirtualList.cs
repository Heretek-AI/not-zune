...
public class LibraryVirtualList : VirtualDatabaseList, ISearchableList
{
		...
		: base(pQueryList, enableSlowDataRequests: true)
	{
		...
		{
			m_owner = owner;
			((VirtualList)this).StoreQueryResults = true;
			...
			((VirtualList)this).Count = m_pQueryList.Count;
			return;
		}
		catch
		{
	...
	public unsafe virtual int SearchForString(string searchString)
	{
		...
		{
			int result;
			fixed (ushort* wszName = &Unsafe.As<char, ushort>(ref global::_003CModule_003E.PtrToStringChars(sorts[0])))
			{
				try
				{
					uint atom = (uint)global::_003CModule_003E.CSchemaMap_002EGetIndex(wszName);
	...
	protected override object OnRequestItem(int index)
	{
		if (!m_fDisposed && m_pQueryList.CheckItemIndex((uint)index))
		{
			...
			libraryDataProviderListItem.SetSlowDataThumbnailExtraction(((VirtualList)this).SlowDataRequestsEnabled);
			m_fItemsAdded = true;
	...
	{
		((LibraryDataProviderListItem)((VirtualList)this)[index]).OnRequestSlowData();
	}

	protected override void InvalidateItem(int index)
	{
		((LibraryDataProviderListItem)((VirtualList)this)[index]).InvalidateAllProperties();
	}

	protected override void OnPropertyChanged(string property)
	{
		((ModelItem)this).OnPropertyChanged(property);
		if (!((ModelItem)this).IsDisposed && property == "Count")
		{
			LibraryDataProviderQueryResult libraryDataProviderQueryResult = (LibraryDataProviderQueryResult)((DataProviderQuery)m_owner).Result;
			if (libraryDataProviderQueryResult != null)
			{
				byte isEmpty = ((((VirtualList)this).Count == 0) ? ((byte)1) : ((byte)0));
