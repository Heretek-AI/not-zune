...
public class MetadataEditMedia
{
	...
	public bool CreationFailed => _creationFailed;
	...
	protected void Initialize(IList mediaList, PropertyDescriptor[] properties)
	{
		...
		{
			_propertyMap[property] = CreatePropertyFromMedia(_mediaList, property);
		}
	}

	public MetadataEditProperty GetProperty(PropertyDescriptor descriptor)
	{
		return _propertyMap[descriptor];
	}

	public MetadataEditProperty GetProperty(string descriptorName)
	{
		MetadataEditProperty result = null;
		foreach (MetadataEditProperty value in _propertyMap.Values)
		{
			if (value.Descriptor.DescriptorName.Equals(descriptorName, StringComparison.InvariantCultureIgnoreCase))
			{
				result = value;
				break;
			}
	...
	public object GetPropertyData(PropertyDescriptor descriptor)
	{
		...
		if (property != null)
		{
			result = property.ConvertToData();
		}
		return result;
	}

	public void SetPropertyData(PropertyDescriptor descriptor, object value)
	{
		GetProperty(descriptor)?.ConvertFromData(value);
	}
	...
	public void ResetExternalErrors()
	{
		...
		foreach (MetadataEditProperty value in _propertyMap.Values)
		{
			value.ExternalError = HRESULT._S_OK;
		}
	}
	...
	public virtual void Commit()
	{
		foreach (PropertyDescriptor property in _properties)
		{
			MetadataEditProperty metadataEditProperty = _propertyMap[property];
			if (metadataEditProperty.Modified)
			{
				SetPropertyToMedia(metadataEditProperty);
			}
		}
		if (!_source.NeedsCommit)
		{
			return;
		}
		foreach (object media in _mediaList)
		{
			_source.Commit(media);
		}
	}
	...
	public virtual bool IsValid()
	{
		foreach (MetadataEditProperty value in _propertyMap.Values)
		{
			if ((value.Modified || value.Required) && !value.Valid)
			{
				return false;
			}
		}
	...
	private MetadataEditProperty CreatePropertyFromMedia(IList mediaList, PropertyDescriptor descriptor)
	{
		string text = descriptor.UnknownString;
		foreach (object media in _mediaList)
		{
			object value = _source.Get(media, descriptor);
			string text2 = descriptor.ConvertToString(value);
			if (text2 == null)
			{
				text2 = descriptor.UnknownString;
			}
			text = AggregateString(text, text2, descriptor);
		}
		return new MetadataEditProperty(descriptor, text);
	}
	...
	private void SetPropertyToMedia(MetadataEditProperty property)
	{
		foreach (object media in _mediaList)
		{
			_source.Set(media, property.Descriptor, property.Descriptor.ConvertFromString(property.Value, property.State));
		}
	}

	protected string AggregateString(string oldString, string newString, PropertyDescriptor descriptor)
	{
		string text = oldString;
		if (text != newString)
		{
			text = ((!(text == descriptor.UnknownString)) ? descriptor.MultiValueString : newString);
		}
		return text;
	}
}
